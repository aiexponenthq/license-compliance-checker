...
ModuleSpec = Tuple[str, Optional[str], Dict[str, object]]
...
class GoDetector(Detector):
    ...
    def discover(self, project_root: Path) -> Sequence[Component]:
        ...
        def register(name: str, version: Optional[str], metadata: Dict[str, object]) -> None:
    ...
    def _parse_go_mod(self, path: Path, module_prefix: Optional[str] = None) -> Iterable[ModuleSpec]:
        lines = path.read_text(encoding="utf-8").splitlines()
        results: List[ModuleSpec] = []
        replace_map: Dict[str, Tuple[str, Optional[str]]] = {}
